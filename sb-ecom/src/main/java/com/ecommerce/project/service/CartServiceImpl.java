...
public class CartServiceImpl implements CartService{
    ...
    public CartDTO addProductToCart(Long productId, Integer quantity) {
        ...
        if(cartItem!=null){
            throw new APIException(product.getProductName() + " Already in Cart");
        }

        if(product.getQuantity() == 0){
            throw new APIException(product.getProductName() + " is sold out");
        }

        if(product.getQuantity() < quantity){
            throw new APIException("Only " + product.getQuantity() + "units left in stock. Please make an order of " +
